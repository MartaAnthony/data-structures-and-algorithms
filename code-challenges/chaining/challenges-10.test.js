...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => {
    response.status(200).send('Hello, darling!');
  });

  app.get('/aboutme', (request, response) => {
    response.status(200).send('Born in Poland , studied in France, lives in the US.');
  });

  app.get('/favoritefoods', (request, response) => {
    response.status(200).send(['chocolate', 'raspberries', 'smoothies', 'sashimi']);
  });
...
const count = (target, input) => {
  ...
  for(var i=0; i<input.length; i++){
    for(var j=0; j<input[i].length;j++){
      endArray.push(input[i][j]);
    }
  }
  let targetArray = endArray.filter(val => val === target
  );
...
const totalSum = (input) => {
  ...
  let sumArray = endArray.reduce((a,b) => a + b);
  return sumArray;
};
